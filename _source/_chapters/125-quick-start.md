...
Since bnd is by design headless, the best way to get start is to use one of the IDEs like bndtools. They have tutorials and an IDE is a more pleasant place than a command line. However, the command line has its advantages so you can also get started bnd  by installing it on the command line. It has lots of commands to try out aspects of bnd.

You can install bnd with its brethren, jpm. Alas, this means you first have to install jpm, fortunately, this is well documented at http://jpm4j.org/#!/md/install. Installing jpm has the other advantage that it provides some useful commands to work with a repository that has all of maven central. 

So assuming you've install jpm, you can install the latest version of bnd as follows:
...
Let's start with something really simple. We have a number of implementation bundles from which we want to create an API. 
